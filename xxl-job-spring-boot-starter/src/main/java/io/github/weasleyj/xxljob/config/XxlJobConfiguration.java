...
import io.github.weasleyj.xxljob.config.XxlJobProperties.JobExecutorProperties;
...
public class XxlJobConfiguration {
    ...
    public XxlJobSpringExecutor xxlJobSpringExecutor(XxlJobProperties jobProperties) {
        if (log.isInfoEnabled()) {
            log.info("Initializing xxl-job configuration.");
        }
        JobExecutorProperties jobExecutor = jobProperties.getExecutor();
        if (null == jobExecutor.getPort() || jobExecutor.getPort() <= 0) {
            jobExecutor.setPort(SocketUtils.findAvailableTcpPort());
        }
        ...
        jse.setAppname(jobExecutor.getAppName());
        jse.setAddress(jobExecutor.getAddress());
        jse.setIp(jobExecutor.getIp());
        jse.setPort(jobExecutor.getPort());
        jse.setLogPath(jobExecutor.getLogPath());
        jse.setLogRetentionDays(jobExecutor.getLogRetentionDays());
        return jse;
    }
